...
let token = "7757387195:AAF63AZcgRjAgzMqTS_bZ8QvwXMG25rkQaI";
...
bot.onText(/\/search (.+)/, async function (msg, match) {
    try {
        ...
        const maxPrice = hasPriceRange ? parseFloat(parts[2]) : Number.MAX_SAFE_INTEGER;

        if (hasPriceRange && (isNaN(minPrice) || isNaN(maxPrice))) {
            bot.sendMessage(chatId, "⚠️ 價格格式錯誤！請使用：/search 關鍵字/最低價/最高價\n範例：/search 手機/3000/8000");
            return;
        }
        ...
        const results = allProducts.filter(p => {
            ...
            const priceInRange = parseFloat(p.price) >= minPrice && parseFloat(p.price) <= maxPrice;
...
bot.onText(/\/question (.+)/, async function (msg, match) {
    try {
        ...
        // Keyword matching
        const matchedFaqs = faqData.filter(item =>
...
bot.onText(/\/getproducts/, async function (msg) {
    ...
    try {
        ...
        const allProducts = [...productJSON.airconditioners, ...productJSON.laptops, ...productJSON.smartphones]; // Merge all products
