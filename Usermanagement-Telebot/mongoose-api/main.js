...
const { AirConditionersCll, LaptopsCll, SmartphonesCll } = require("./models/productsSchema.js"); // Ensure path is correct
...
app.put('/airconditioners/:id', async (req, res) => {
    try {
        const airconditioners = await AirConditionersCll.findByIdAndUpdate(req.params.id, req.body, {
            new: true,
            runValidators: true,
        });
        if (!airconditioners) {
...
app.delete('/airconditioners/:id', async (req, res) => {
    try {
        const airconditioner = await AirConditionersCll.findByIdAndDelete(req.params.id);
        if (!airconditioner) {
            return res.status(404).send({ message: "Air conditioner not found." });
        }
        res.status(200).send({ message: "Air conditioner deleted successfully.", deletedItem: airconditioner });
    } catch (error) {
        res.status(500).send({ message: "An error occurred while deleting the air conditioner.", error });
    }
});
...
app.delete('/laptops/:id', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: "Laptop deleted successfully.", deletedItem: laptop });
    } catch (error) {
        res.status(500).send({ message: "An error occurred while deleting the Laptop", error });
    }
});
...
app.put('/smartphones/:id', async (req, res) => {
    try {
        const smartphone = await SmartphonesCll.findByIdAndUpdate(req.params.id, req.body, {
            new: true,
            runValidators: true,
        });
        if (!smartphone) {
...
app.delete('/smartphones/:id', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: "Smartphone deleted successfully.", deletedItem: smartphone });
    } catch (error) {
        res.status(500).send({ message: "An error occurred while deleting the Smartphone", error });
    }
});
...
app.put('/shops/:id', async (req, res) => {
    try {
        const shop = await ShopsCll.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
app.delete('/shops/:id', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: "Shop deleted successfully.", deletedItem: shop });
    } catch (error) {
        res.status(500).send({ message: "An error occurred while deleting the Shop", error });
    }
});
...
app.put('/questions/:id', async (req, res) => {
    try {
        const question = await QuestionsCll.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
app.delete('/questions/:id', async (req, res) => {
    try {
        ...
        res.status(200).send({ message: "Question deleted successfully.", deletedItem: question });
    } catch (error) {
        res.status(500).send({ message: "An error occurred while deleting the Question", error });
    }
});
